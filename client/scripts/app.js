...
App.prototype.fetch = function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      that.storeAndDisplayNewMessages(data);
...
App.prototype.storeAndDisplayNewMessages = function (messagesArr) {
  for (let i = 0; i < messagesArr.length; i++) {
    let id = messagesArr[i].id;
    //TODO: ensure order of date and then object id after
    if (this.messages[id] === undefined) {
      // sanitize message
      this.messages[id] = this.sanitizeMessage(messagesArr[i]);
      //add if new room
      this.messages[id].roomname = this.replaceIfBlankRoomName(this.messages[id].roomname.trim(), 'toDescriptor');
      let roomName = this.messages[id].roomname;
  ...
  const renderedTemplate = templ(
    {
      ...
      date: moment(Date(parseInt(msg.createdAt))).format('llll'),
