    ...
    request({
      ...
      request({
        ...
        dbConnection.query(queryString, queryArgs, function(err, results) {
          ...
          expect(results[results.length - 1].text).to.equal('In mercy\'s name, three days is all I need.');
  ...
  it('Should output all messages from the DB', function(done) {
    ...
    dbConnection.query(queryString, queryArgs, function(err) {
      ...
      request('http://127.0.0.1:3000/classes/messages', function(error, response, body) {
        ...
        expect(messageLog[messageLog.length - 1].text).to.equal('Men like you can never change!');
        expect(messageLog[messageLog.length - 1].roomname).to.equal('main');
        done();
      });
    });
  });
